...
When(`User inputs valid email address`, () => {
    cy.get(`input[data-test='email.input']`).type('carmellajoy.ventanilla22@gmail.com');
});
...
When(`User types invalid email ex."carmella.ventanilla@gmail.com"`, () => {
    cy.get(`input[data-test='email.input']`).type('carmella.ventanilla@gmail.com');
});
...
When(`User inputs valid email address`, () => {
    cy.get(`input[data-test='email.input']`).clear().type('carmellajoy.ventanilla22@gmail.com');
});
And(`clicks Sign In button`, () => {
    cy.get(`button[type="submit"]`).click();
});
...
Then('Message "Invalid email or password." appears', () => {
    cy.get('li[class="chakra-toast"] p[class="chakra-text css-s6fle"]').should('exist')
});
